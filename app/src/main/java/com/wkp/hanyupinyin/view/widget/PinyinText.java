package com.wkp.hanyupinyin.view.widget;
...
//            }
        ...
        // +10来避免拼音显示不全的问题
        setMeasuredDimension(measuredWidth + paddingLeft + paddingRight + 14, measuredHeight + paddingTop + paddingBottom + 10);
    }
    ...
    private void drawPinyinAndText(Canvas canvas) {
        ...
        for (int i = 0; i < mPinyinCompats.size(); i++) {
            ...
            Rect textRect = copyRect(compat.textRect);
            Rect pinyinRect = copyRect(compat.pinyinRect);
            // draw text
            ...
            // 文字居中
            int textHalfWidth = (textRect.width() - getTextWidth(compat.text, mTextSize)) >> 1;
            canvas.drawText(compat.text, textRect.left + textHalfWidth, textRect.bottom, mPaint);
            ...
            // 拼音文字居中
            int pinyinTextHalfWidth = (pinyinRect.width() - getTextWidth(compat.pinyin, mPinyinTextSize)) >> 1;
            canvas.drawText(compat.pinyin, pinyinRect.left + pinyinTextHalfWidth, pinyinRect.bottom, mPaint);
        }
    }
